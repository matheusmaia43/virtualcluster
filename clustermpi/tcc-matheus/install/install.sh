...
path_cg=/opt/lampp/htdocs/clustermpi/tcc-matheus/configura_cluster
#-------------------------------------------------------#
function criaVMS(){
  $path_c/createVMS.sh -v $qtd_nodes $cpu $men
  verificaStatus
}
function troca(){
    ...
    senha_mpi="mpiuser"
    echo "Trocando chaves..."
    sshpass -p $senha_mpi ssh mpiuser@$ips "cd /home/mpiuser/configura_vm/ && ./configuraCHAVES.sh -t"
    done
}
